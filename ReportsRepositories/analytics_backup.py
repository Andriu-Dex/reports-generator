...
class GitHubAnalytics:
    ...
    def filtrar_datos(self, fecha_inicio=None, fecha_fin=None, autores=None, 
                     etiquetas=None, estado=None, tipo=None, busqueda_texto=None):
        """
        ...
        if autores:
            ...
            mask = df_filtrado['Autores_Lista'].apply(
                lambda x: any(autor in x for autor in autores)
            )
        ...
        if etiquetas:
            ...
            mask = df_filtrado['Etiquetas_Lista'].apply(
                lambda x: any(etiqueta in x for etiqueta in etiquetas)
            )
            df_filtrado = df_filtrado[mask]
        ...
        if busqueda_texto:
            mask = (
                df_filtrado['Título'].str.contains(busqueda_texto, case=False, na=False) |
                df_filtrado['Descripción'].str.contains(busqueda_texto, case=False, na=False)
            )
            df_filtrado = df_filtrado[mask]
        
        return df_filtrado.to_dict('records')
    
    def exportar_datos_filtrados(self, datos_filtrados, output_dir="reportes", formato="excel"):
        ...
        if formato == "excel":
            ...
            filename = f"reporte_filtrado_{timestamp}.csv"
            filepath = os.path.join(output_dir, filename)
            df_filtrado.to_csv(filepath, index=False, encoding='utf-8')
        elif formato == "json":
            filename = f"reporte_filtrado_{timestamp}.json"
            filepath = os.path.join(output_dir, filename)
            df_filtrado.to_json(filepath, orient='records', indent=2)
    ...
    def obtener_estadisticas_resumen(self):
        ...
        # Período de datos
        fecha_min = self.df['Creado'].min()
        fecha_max = self.df['Creado'].max()
