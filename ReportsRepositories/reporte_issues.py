...
def generar_pdf_desde_markdown():
    ...
    try:
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(doc.leftMargin, doc.height + doc.topMargin - 10, f"Reporte GitHub: {REPO}")
            ...
            canvas.setFont('Helvetica', 8)
            canvas.drawString(doc.leftMargin, doc.bottomMargin - 20, f"Página {page_num}")
        ...
        elements.append(Spacer(1, 0.5*inch))
        elements.append(Paragraph(
            f"Generado el {pd.Timestamp.now().strftime('%d de %B de %Y, %H:%M')}",
            styles['DateStyle']
        ))
        ...
        issue_count = {'issue_open': 0, 'issue_closed': 0, 'pr_open': 0, 'pr_closed': 0}
        ...
        elements.append(Paragraph("Resumen del Repositorio", styles['CustomHeading3']))
        ...
        if sum(issue_count.values()) > 0:
            ...
            porcentaje_abiertos = (issue_count['issue_open'] + issue_count['pr_open']) / sum(issue_count.values()) * 100
            porcentaje_cerrados = (issue_count['issue_closed'] + issue_count['pr_closed']) / sum(issue_count.values()) * 100
            
            elements.append(Paragraph(
                f"<b>Estado del repositorio:</b> {porcentaje_abiertos:.1f}% abiertos, {porcentaje_cerrados:.1f}% cerrados",
                styles['CustomNormal']
            ))
        ...
        # === ÍNDICE DE CONTENIDO ===
        elements.append(Paragraph("Índice de Contenido", styles['CustomTitle']))
        elements.append(Spacer(1, 0.25*inch))
        
        # Crear un índice simple
        if issue_count['issue_open'] > 0:
            elements.append(Paragraph("1. Issues Abiertos", styles['CustomHeading3']))
        if issue_count['issue_closed'] > 0:
            elements.append(Paragraph("2. Issues Cerrados", styles['CustomHeading3']))
        if issue_count['pr_open'] > 0:
            elements.append(Paragraph("3. Pull Requests Abiertos", styles['CustomHeading3']))
        if issue_count['pr_closed'] > 0:
            elements.append(Paragraph("4. Pull Requests Cerrados", styles['CustomHeading3']))
        ...
        for line in lines:
            if line.strip():
                ...
                if line.startswith('# '):
                    ...
                    # Si cambiamos de tipo de sección, agregar encabezado de sección
                    if current_section != last_section:
                        section_counter += 1
                        if current_section == "issue_open":
                            elements.append(Paragraph(f"{section_counter}. Issues Abiertos", styles['CustomTitle']))
                        elif current_section == "issue_closed":
                            elements.append(Paragraph(f"{section_counter}. Issues Cerrados", styles['CustomTitle']))
                        elif current_section == "pr_open":
                            elements.append(Paragraph(f"{section_counter}. Pull Requests Abiertos", styles['CustomTitle']))
                        elif current_section == "pr_closed":
                            elements.append(Paragraph(f"{section_counter}. Pull Requests Cerrados", styles['CustomTitle']))
                    ...
                    metadata_style = ParagraphStyle(
                        ...
                        backColor=colors.Color(0.97, 0.97, 0.97),  # Gris muy claro
                        borderPadding=6,
                        borderWidth=0.5,
                        borderColor=colors.lightgrey,
                        borderRadius=4
                    ...
                    elements.append(Paragraph(metadata_text, metadata_style))
                        ...
                        if line.strip() == '---':
                            ...
                            # Añadir formato para enlaces si existen - con mejor manejo de caracteres especiales
                            if "http://" in line or "https://" in line:
                                try:
                                    # Patrón de URL más seguro
                                    pattern = r'(https?://[^\s"\'<>]+)'
                                    # Reemplazar URLs sin caracteres problemáticos
                                    line = re.sub(pattern, lambda m: f'<link href="{m.group(1).strip()}">{m.group(1).strip()}</link>', line)
                                except:
                                    # Si hay error en el procesamiento de enlaces, mostrar texto plano
                                    pass
