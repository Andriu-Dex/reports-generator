...
class GithubReportApp(ctk.CTk):
    def __init__(self):
        ...
        self.output_dir = ctk.StringVar(value=os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "reportes"))
        self.status_text = ctk.StringVar(value="Listo para generar reportes...")
    ...
    def create_config_tab(self):
        # --- Configuración ---
        ctk.CTkLabel(self.config_tab, text="Configuración del Reporte", font=("Segoe UI", 22, "bold")).pack(pady=(10, 20))
        form = ctk.CTkFrame(self.config_tab, fg_color=("#f5f6fa", "#222831"))
        form.pack(padx=30, pady=10, fill="x")

        # Token
        ctk.CTkLabel(form, text="Token de GitHub:", anchor="w").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        self.token_entry = ctk.CTkEntry(form, textvariable=self.github_token, width=350, show="*")
        self.token_entry.grid(row=0, column=1, padx=10, pady=10, sticky="w")
        self.show_token_btn = ctk.CTkButton(form, text="👁", width=40, command=self.toggle_password_visibility)
        self.show_token_btn.grid(row=0, column=2, padx=5, pady=10)

        # Repo
        ctk.CTkLabel(form, text="Repositorio:", anchor="w").grid(row=1, column=0, padx=10, pady=10, sticky="w")
        ctk.CTkEntry(form, textvariable=self.repo, width=350).grid(row=1, column=1, padx=10, pady=10, sticky="w")
        ctk.CTkLabel(form, text="(usuario/repo)").grid(row=1, column=2, padx=5, pady=10, sticky="w")

        # Output dir
        ctk.CTkLabel(form, text="Directorio de reportes:", anchor="w").grid(row=2, column=0, padx=10, pady=10, sticky="w")
        ctk.CTkEntry(form, textvariable=self.output_dir, width=350).grid(row=2, column=1, padx=10, pady=10, sticky="w")
        ctk.CTkButton(form, text="Buscar", command=self.browse_output_dir, width=80).grid(row=2, column=2, padx=5, pady=10)

        # PDF option
        ctk.CTkCheckBox(form, text="Generar reporte en PDF", variable=self.enable_pdf).grid(row=3, column=0, columnspan=2, padx=10, pady=10, sticky="w")
        ctk.CTkLabel(form, text="Para PDF: pip install reportlab markdown2", text_color="#888").grid(row=3, column=2, padx=5, pady=10, sticky="w")

        # Botones acción
        btns = ctk.CTkFrame(self.config_tab, fg_color="transparent")
        btns.pack(pady=20)
        ctk.CTkButton(btns, text="Guardar Configuración", command=self.save_config, fg_color="#0078D4", hover_color="#005fa3").pack(side="left", padx=10)
        ctk.CTkButton(btns, text="Generar Reportes", command=self.generate_reports, fg_color="#43a047", hover_color="#357a38").pack(side="left", padx=10)

        # Estado
        status = ctk.CTkFrame(self.config_tab, fg_color="transparent")
        status.pack(fill="x", pady=(10, 0), padx=30)
        ctk.CTkLabel(status, textvariable=self.status_text, text_color="#0078D4").pack(side="left", padx=5)
    ...
    def create_analytics_tab(self):
        ...
        ctk.CTkLabel(self.analytics_tab, text="📊 Dashboard de Analytics", font=("Segoe UI", 22, "bold")).pack(pady=(10, 20))
        
        # Botones de control
        control_frame = ctk.CTkFrame(self.analytics_tab)
        control_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkButton(control_frame, text="🔄 Cargar Datos", command=self.load_analytics_data, 
                     fg_color="#0078D4", hover_color="#005fa3").pack(side="left", padx=10, pady=10)
        ctk.CTkButton(control_frame, text="📈 Generar Dashboard", command=self.generate_dashboard,
                     fg_color="#43a047", hover_color="#357a38").pack(side="left", padx=10, pady=10)
        ctk.CTkButton(control_frame, text="🌐 Abrir Dashboard", command=self.open_dashboard,
                     fg_color="#ff9800", hover_color="#f57c00").pack(side="left", padx=10, pady=10)
        
        # Área de estadísticas rápidas
        stats_frame = ctk.CTkFrame(self.analytics_tab)
        stats_frame.pack(fill="x", padx=20, pady=10)
        
        ctk.CTkLabel(stats_frame, text="📊 Estadísticas Rápidas", font=("Segoe UI", 16, "bold")).pack(pady=10)
        ...
        ctk.CTkLabel(stats_grid, text="Total Issues:", font=("Segoe UI", 12, "bold")).grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(stats_grid, textvariable=self.total_issues_var, text_color="#e53935").grid(row=0, column=1, padx=10, pady=5, sticky="w")
        
        ctk.CTkLabel(stats_grid, text="Total PRs:", font=("Segoe UI", 12, "bold")).grid(row=0, column=2, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(stats_grid, textvariable=self.total_prs_var, text_color="#43a047").grid(row=0, column=3, padx=10, pady=5, sticky="w")
        
        ctk.CTkLabel(stats_grid, text="Contribuyentes Activos:", font=("Segoe UI", 12, "bold")).grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(stats_grid, textvariable=self.active_contributors_var, text_color="#0078D4").grid(row=1, column=1, padx=10, pady=5, sticky="w")
        
        ctk.CTkLabel(stats_grid, text="Tiempo Promedio de Resolución:", font=("Segoe UI", 12, "bold")).grid(row=1, column=2, padx=10, pady=5, sticky="w")
        ctk.CTkLabel(stats_grid, textvariable=self.avg_resolution_time_var, text_color="#ff9800").grid(row=1, column=3, padx=10, pady=5, sticky="w")
        
        # Área de información
        self.analytics_info = ctk.CTkTextbox(self.analytics_tab, height=200, font=("Segoe UI", 12))
        self.analytics_info.pack(fill="both", expand=True, padx=20, pady=10)
        self.analytics_info.insert("0.0", "💡 Información de Analytics\n\n")
        self.analytics_info.insert("end", "1. Haga clic en 'Cargar Datos' para cargar los datos del repositorio\n")
        self.analytics_info.insert("end", "2. Use 'Generar Dashboard' para crear visualizaciones interactivas\n")
        self.analytics_info.insert("end", "3. Abra el dashboard en su navegador con 'Abrir Dashboard'\n\n")
        self.analytics_info.insert("end", "📈 El dashboard incluye:\n")
        self.analytics_info.insert("end", "• Análisis de tendencias temporales\n")
        self.analytics_info.insert("end", "• Gráficos de contribuyentes activos\n")
        self.analytics_info.insert("end", "• Distribución de tiempos de resolución\n")
        self.analytics_info.insert("end", "• Visualización de etiquetas populares\n")
        self.analytics_info.insert("end", "• Análisis de estados de issues y PRs\n")
        self.analytics_info.configure(state="disabled")

    def create_filters_tab(self):
        """Crear la pestaña de filtros avanzados"""
        ctk.CTkLabel(self.filters_tab, text="🔍 Filtros Avanzados", font=("Segoe UI", 22, "bold")).pack(pady=(10, 20))
        ...
        ctk.CTkLabel(date_frame, text="📅 Filtros de Fecha", font=("Segoe UI", 16, "bold")).pack(pady=(10, 5))
        
        date_grid = ctk.CTkFrame(date_frame)
        date_grid.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(date_grid, text="Fecha Inicio:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkEntry(date_grid, textvariable=self.fecha_inicio, placeholder_text="YYYY-MM-DD", width=150).grid(row=0, column=1, padx=10, pady=5)
        
        ctk.CTkLabel(date_grid, text="Fecha Fin:").grid(row=0, column=2, padx=10, pady=5, sticky="w")
        ctk.CTkEntry(date_grid, textvariable=self.fecha_fin, placeholder_text="YYYY-MM-DD", width=150).grid(row=0, column=3, padx=10, pady=5)
        
        # Botones de fecha predefinidos
        date_buttons = ctk.CTkFrame(date_frame)
        date_buttons.pack(fill="x", padx=10, pady=5)
        
        ctk.CTkButton(date_buttons, text="🗓️ Último Mes", command=lambda: self.set_date_filter(30), width=100).pack(side="left", padx=5)
        ctk.CTkButton(date_buttons, text="📆 Últimos 3 Meses", command=lambda: self.set_date_filter(90), width=120).pack(side="left", padx=5)
        ctk.CTkButton(date_buttons, text="📅 Último Año", command=lambda: self.set_date_filter(365), width=100).pack(side="left", padx=5)
        ctk.CTkButton(date_buttons, text="🔄 Limpiar", command=self.clear_date_filters, width=80).pack(side="left", padx=5)
        
        # Filtros de contenido
        content_frame = ctk.CTkFrame(main_filter_frame)
        content_frame.pack(fill="x", padx=10, pady=10)
        ctk.CTkLabel(content_frame, text="📝 Filtros de Contenido", font=("Segoe UI", 16, "bold")).pack(pady=(10, 5))
        
        content_grid = ctk.CTkFrame(content_frame)
        content_grid.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(content_grid, text="Autor:").grid(row=0, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkEntry(content_grid, textvariable=self.filtro_autor, placeholder_text="nombre_usuario", width=150).grid(row=0, column=1, padx=10, pady=5)
        
        ctk.CTkLabel(content_grid, text="Etiqueta:").grid(row=0, column=2, padx=10, pady=5, sticky="w")
        ctk.CTkEntry(content_grid, textvariable=self.filtro_etiqueta, placeholder_text="bug, feature, etc.", width=150).grid(row=0, column=3, padx=10, pady=5)
        
        ctk.CTkLabel(content_grid, text="Estado:").grid(row=1, column=0, padx=10, pady=5, sticky="w")
        ctk.CTkComboBox(content_grid, variable=self.filtro_estado, values=["Todos", "open", "closed"], width=150).grid(row=1, column=1, padx=10, pady=5)
        
        ctk.CTkLabel(content_grid, text="Tipo:").grid(row=1, column=2, padx=10, pady=5, sticky="w")
        ctk.CTkComboBox(content_grid, variable=self.filtro_tipo, values=["Todos", "Issue", "Pull Request"], width=150).grid(row=1, column=3, padx=10, pady=5)
        
        # Búsqueda de texto
        search_frame = ctk.CTkFrame(content_frame)
        search_frame.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(search_frame, text="🔍 Búsqueda en Texto:").pack(side="left", padx=10)
        ctk.CTkEntry(search_frame, textvariable=self.busqueda_texto, placeholder_text="Buscar en títulos y descripción...", width=300).pack(side="left", padx=10)
        
        # Botones de acción
        action_frame = ctk.CTkFrame(main_filter_frame)
        action_frame.pack(fill="x", padx=10, pady=20)
        
        ctk.CTkButton(action_frame, text="🔍 Aplicar Filtros", command=self.apply_filters, 
                     fg_color="#0078D4", hover_color="#005fa3").pack(side="left", padx=10)
        ctk.CTkButton(action_frame, text="🧹 Limpiar Filtros", command=self.clear_filters,
                     fg_color="#6c757d", hover_color="#5a6268").pack(side="left", padx=10)
        ctk.CTkButton(action_frame, text="💾 Exportar Datos Filtrados", command=self.export_filtered_data,
        ...
        ctk.CTkLabel(results_filter_frame, text="📊 Resultados de Filtros", font=("Segoe UI", 16, "bold")).pack(pady=10)
        
        self.filter_results = ctk.CTkTextbox(results_filter_frame, height=150, font=("Segoe UI", 12))
        self.filter_results.pack(fill="both", expand=True, padx=10, pady=10)
        self.filter_results.insert("0.0", "Los resultados de los filtros aplicados aparecerán aquí.\n\n")
        self.filter_results.insert("end", "Use los controles superiores para configurar filtros y haga clic en 'Aplicar Filtros'.")
        self.filter_results.configure(state="disabled")

    def create_results_tab(self):
        """Crear la pestaña de resultados mejorada"""
        ctk.CTkLabel(self.results_tab, text="📈 Resultados y Análisis", font=("Segoe UI", 18, "bold")).pack(pady=10, anchor="w")
        
        # Área de resultados principal
        self.results_area = ctk.CTkTextbox(self.results_tab, height=350, font=("Consolas", 13), wrap="word")
        ...
        ctk.CTkButton(btns2, text="📊 Abrir Excel", command=lambda: self.open_file("excel"), fg_color="#0078D4").pack(side="left", padx=10)
        ctk.CTkButton(btns2, text="📝 Abrir Markdown", command=lambda: self.open_file("markdown"), fg_color="#6d4aff").pack(side="left", padx=10)
        ctk.CTkButton(btns2, text="📄 Abrir PDF", command=lambda: self.open_file("pdf"), fg_color="#e53935").pack(side="left", padx=10)
        ctk.CTkButton(btns2, text="📈 Ver Dashboard", command=self.open_dashboard, fg_color="#ff9800").pack(side="left", padx=10)

    def create_help_tab(self):
        """Crear la pestaña de ayuda"""
        ctk.CTkLabel(self.help_tab, text="Ayuda y Documentación", font=("Segoe UI", 20, "bold")).pack(pady=10, anchor="w")
        ayuda = ctk.CTkTextbox(self.help_tab, font=("Segoe UI", 13), wrap="word")
...
""")
    ...
    def load_analytics_data(self):
        ...
        if not self.github_token.get() or not self.repo.get():
            messagebox.showerror("Error", "Debe configurar el token y repositorio primero.")
            return
        ...
        try:
            ...
            self.datos_actuales = reporte_issues.procesar_reporte(issues_y_prs, commits)
            ...
            self.analytics_info.insert("0.0", f"✅ Datos cargados exitosamente!\n\n")
            self.analytics_info.insert("end", f"📊 Repositorio: {self.repo.get()}\n")
            self.analytics_info.insert("end", f"📈 Total de elementos: {len(self.datos_actuales)}\n")
            self.analytics_info.insert("end", f"🗓️ Datos cargados: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            self.analytics_info.insert("end", "🎯 Ahora puede:\n")
            self.analytics_info.insert("end", "• Generar dashboard interactivo\n")
    ...
    def update_quick_stats(self):
        ...
        prs = [d for d in self.datos_actuales if d.get("Tipo") == "Pull Request"]
        ...
        tiempos_resolucion = []
        for item in self.datos_actuales:
            if item.get("Estado") == "closed" and item.get("Fecha_Cerrado") and item.get("Fecha_Creacion"):
                try:
                    creado = datetime.fromisoformat(item["Fecha_Creacion"].replace('Z', '+00:00'))
                    cerrado = datetime.fromisoformat(item["Fecha_Cerrado"].replace('Z', '+00:00'))
        ...
        avg_resolution = np.mean(tiempos_resolucion) if tiempos_resolucion else 0
    ...
    def generate_dashboard(self):
        ...
        if not self.analytics or not self.datos_actuales:
            messagebox.showwarning("Advertencia", "Debe cargar los datos primero.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Éxito", f"Dashboard generado en: {dashboard_path}")
            
        except Exception as e:
            self.status_text.set(f"Error al generar dashboard: {e}")
            self.progress.set(0)
            messagebox.showerror("Error", f"No se pudo generar el dashboard: {e}")
    
    def open_dashboard(self):
        """Abrir dashboard en el navegador"""
        dashboard_path = os.path.join(self.output_dir.get(), "github_analytics_dashboard.html")
        ...
        if os.path.exists(dashboard_path):
            ...
            messagebox.showwarning("Advertencia", "Dashboard no encontrado. Genérelo primero.")
    ...
    def clear_filters(self):
        ...
        self.filter_results.insert("0.0", "Filtros limpiados. Configure nuevos filtros y haga clic en 'Aplicar Filtros'.")
        self.filter_results.configure(state="disabled")
    ...
    def apply_filters(self):
        ...
        try:
            ...
            # Aplicar filtros
            filtered_data = self.analytics.filter_data(self.datos_actuales, filters)
    ...
    def show_filter_results(self, filtered_data, filters):
        ...
        for key, value in filters.items():
            ...
            self.filter_results.insert("end", f"  • {filter_names.get(key, key)}: {value}\n")
        
        self.filter_results.insert("end", f"\n📈 Resultados encontrados: {len(filtered_data)}\n\n")
        ...
        if filtered_data:
            ...
            cerrados = [d for d in filtered_data if d.get("Estado") == "closed"]
            
            self.filter_results.insert("end", "📊 Distribución por tipo:\n")
            self.filter_results.insert("end", f"  • Issues: {len(issues)}\n")
            self.filter_results.insert("end", f"  • Pull Requests: {len(prs)}\n\n")
            
            self.filter_results.insert("end", "📊 Distribución por estado:\n")
            self.filter_results.insert("end", f"  • Abiertos: {len(abiertos)}\n")
            self.filter_results.insert("end", f"  • Cerrados: {len(cerrados)}\n\n")
            ...
            top_autores = sorted(autores.items(), key=lambda x: x[1], reverse=True)[:5]
            
            self.filter_results.insert("end", "👥 Top 5 autores:\n")
            for autor, count in top_autores:
                self.filter_results.insert("end", f"  • {autor}: {count} elementos\n")
        else:
            self.filter_results.insert("end", "❌ No se encontraron datos que coincidan con los filtros aplicados.\n")
    ...
    def export_filtered_data(self):
        ...
        try:
            ...
            export_path = self.analytics.export_filtered_data(filtered_data, self.output_dir.get())
    ...
    def browse_output_dir(self):
        directory = filedialog.askdirectory(initialdir=self.output_dir.get(), title="Seleccionar directorio para reportes")
        if directory:
    ...
    def run_report_generation(self):
        try:
            self.after(0, lambda: self.update_status("Obteniendo issues y PRs..."))
            issues_y_prs = reporte_issues.obtener_issues_y_prs()
    ...
    def show_results(self, total_items, filas):
        ...
        self.results_area.insert("end", f"RESUMEN DEL REPOSITORIO: {self.repo.get()}\n")
        self.results_area.insert("end", "="*50 + "\n\n")
        self.results_area.insert("end", f"Total de elementos analizados: {total_items}\n\n")
        ...
        self.results_area.insert("end", f"  - Total: {issues_abiertos + issues_cerrados}\n\n")
        ...
        self.results_area.insert("end", f"  - Total: {prs_abiertos + prs_cerrados}\n\n")
        self.results_area.insert("end", "ARCHIVOS GENERADOS:\n")
        self.results_area.insert("end", f"  - Excel: {os.path.join(self.output_dir.get(), 'reporte_completo.xlsx')}\n")
        self.results_area.insert("end", f"  - Markdown: {os.path.join(self.output_dir.get(), 'reporte_completo.md')}\n")
        if self.enable_pdf.get():
            self.results_area.insert("end", f"  - PDF: {os.path.join(self.output_dir.get(), 'reporte_completo.pdf')}\n")
        self.results_area.configure(state="disabled")
